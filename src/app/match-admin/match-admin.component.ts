...
export class MatchAdminComponent {
  ...
  async loadMatch(id: string) {
    try {
      ...
      if (data.scheduled_at) {
        ...
        this.editDate.set(`${year}-${month}-${day}T${hours}:${minutes}`);
      }
  ...
  async saveSettings() {
    try {
      ...
      // Añadimos el offset de Colombia (-05:00) al guardar
      const dateWithOffset = this.editDate() ? `${this.editDate()}:00-05:00` : null;
      
      await this.supabase.updateMatchState(this.matchId(), {
        scheduled_at: dateWithOffset as any
      });
